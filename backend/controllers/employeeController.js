...
async function getEmployeeRow(userId) {
  const [[employee]] = await pool.execute(
    `SELECT e.employee_id, e.works_at,
     ...
     WHERE e.employee_account = ? LIMIT 1`,
    [userId]
  );
...
const getDriverDashboard = async (req, res) => {
  try {
    ...
    if (!employee) return res.status(404).json({ message: "Employee profile not found. Contact your administrator." });
 
    const [recentEvents] = await pool.execute(
      `SELECT pte.pkg_tracking_event_id, pte.event_time,
              ps.status_name, ps.status_type,
              p.package_id, p.pkg_weight, s.tracking_number,
              a.city AS location_name,
       ...
       JOIN address        a  ON pte.happened_at    = a.address_id
    ...
    res.json({
      ...
      stats: { total_scans: counts.total_scans },
...
const getHandlerDashboard = async (req, res) => {
  try {
    ...
    const [recentEvents] = await pool.execute(
      `SELECT pte.pkg_tracking_event_id, pte.event_time,
              ps.status_name, ps.status_type,
              p.package_id, p.pkg_weight, p.pkg_length, p.pkg_width, p.pkg_height,
              s.tracking_number, a.city AS location_name
       FROM package_tracking_event pte
       JOIN package        p  ON pte.for_package    = p.package_id
       JOIN shipment       s  ON p.part_of_shipment = s.shipment_id
       JOIN package_status ps ON pte.event_status   = ps.status_no
    ...
    const [statusCounts] = await pool.execute(
      `SELECT ps.status_type, COUNT(*) AS cnt
       FROM package_tracking_event pte
       JOIN package_status ps ON pte.event_status = ps.status_no
       WHERE pte.handled_by = ? GROUP BY ps.status_type`,
      [employee.employee_id]
    );
    res.json({
      employee: {
  ...
  try {
    ...
    const [recentShipments] = await pool.execute(
      `SELECT s.shipment_id, s.tracking_number, s.receiver_name,
              p.total_paid, p.date_paid, u.name AS customer_name,
    ...
    const [pendingList] = await pool.execute(
      `SELECT ctr.request_id, ctr.business_name, ctr.created_at, u.name, u.email
       ...
       JOIN users    u ON c.cust_account = u.user_id
       WHERE ctr.status = 'pending' ORDER BY ctr.created_at ASC`
    );
 
    const [facilities] = await pool.execute(
      `SELECT facility_id, facility_name, department_type FROM facility ORDER BY facility_name ASC`
    );
...
const addEmployee = async (req, res) => {
  ...
  try {
    ...
    if (dupEmail.length > 0) { await conn.rollback(); return res.status(409).json({ message: "Email is already in use." }); }
    const [dupPhone] = await conn.execute(`SELECT user_id FROM users WHERE phone_num = ?`, [phone]);
    if (dupPhone.length > 0) { await conn.rollback(); return res.status(409).json({ message: "Phone number is already in use." }); }
    ...
    await conn.execute(
      `INSERT INTO employee (works_at, employee_account) VALUES (?, ?)`,
...
const editEmployee = async (req, res) => {
  ...
  try {
    await conn.beginTransaction();
    const [[target]] = await conn.execute(`SELECT user_id, role FROM users WHERE user_id = ? LIMIT 1`, [targetUserId]);
    if (!target) { await conn.rollback(); return res.status(404).json({ message: "User not found." }); }
    if (target.role === "customer") { await conn.rollback(); return res.status(400).json({ message: "Cannot edit customer accounts from this endpoint." }); }
 
    if (email) {
      const [dup] = await conn.execute(`SELECT user_id FROM users WHERE email = ? AND user_id <> ?`, [email, targetUserId]);
      if (dup.length > 0) { await conn.rollback(); return res.status(409).json({ message: "Email is already in use." }); }
    }
    if (phone) {
      const [dup] = await conn.execute(`SELECT user_id FROM users WHERE phone_num = ? AND user_id <> ?`, [phone, targetUserId]);
      if (dup.length > 0) { await conn.rollback(); return res.status(409).json({ message: "Phone number is already in use." }); }
    }
    ...
    if (phone) { userFields.push("phone_num = ?"); userValues.push(phone); }
    if (role)  { userFields.push("role = ?");      userValues.push(role); }
    if (password) {
      if (password.length < 8) { await conn.rollback(); return res.status(400).json({ message: "Password must be at least 8 characters." }); }
      const hashed = await bcrypt.hash(password, 10);
    ...
    if (userFields.length > 0) {
      ...
      await conn.execute(`UPDATE users SET ${userFields.join(", ")} WHERE user_id = ?`, userValues);
    }
    if (facilityId) {
      await conn.execute(`UPDATE employee SET works_at = ? WHERE employee_account = ?`, [facilityId, targetUserId]);
    }
    await conn.commit();
...
const deleteEmployee = async (req, res) => {
  ...
  try {
    ...
    const [[target]] = await conn.execute(`SELECT user_id, role FROM users WHERE user_id = ? LIMIT 1`, [targetUserId]);
    if (!target) { await conn.rollback(); return res.status(404).json({ message: "User not found." }); }
    if (target.role === "customer") { await conn.rollback(); return res.status(400).json({ message: "Cannot delete customer accounts from this endpoint." }); }
