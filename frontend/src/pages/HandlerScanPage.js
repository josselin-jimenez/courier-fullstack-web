...
function HandlerScanPage() {
  ...
  useEffect(() => {
    getPackageStatuses("handler")
      .then(setStatuses)
      .catch(() => setError("Failed to load handler statuses."));
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await submitHandlerScan(selectedPackageId, selectedStatus);
  ...
  return (
    <Container maxWidth="md" sx={{ py: 5 }}>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Search by tracking number, select a package, and record a handler scan.
      </Typography>
      ...
      {packages.length > 0 && (
        <Paper sx={{ p: 3 }}>
          ...
          <Button variant="contained" onClick={handleSubmit} disabled={loadingSubmit}>
            {loadingSubmit ? "Submitting..." : "Submit Handler Scan"}
          </Button>
...
export default HandlerScanPage;